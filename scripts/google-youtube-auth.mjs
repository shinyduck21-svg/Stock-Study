...
const credentialsPath = resolve(rootDir, 'youtube-credentials.json');
const tokenPath = resolve(rootDir, 'youtube-token.json');
const scope = 'https://www.googleapis.com/auth/youtube.upload';
const credentials = JSON.parse(readFileSync(credentialsPath, 'utf8')).installed;

if (!credentials) throw new Error('youtube-credentials.json must contain an "installed" OAuth client.');
...
console.log('\nOpen this URL and approve access to the target YouTube channel:\n');
console.log(authUrl.toString());
console.log('\nCopy the full redirected URL, or only its code= value.\n');

const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
const answer = (await rl.question('Redirected URL or code: ')).trim();
rl.close();
const code = extractCode(answer);
if (!code) throw new Error('Could not find an authorization code.');
...
if (!token.refresh_token) throw new Error('Google did not return a refresh_token. Re-run and approve the consent screen.');
